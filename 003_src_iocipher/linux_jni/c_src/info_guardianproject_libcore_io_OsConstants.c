...
Java_info_guardianproject_libcore_io_OsConstants_initConstants(JNIEnv *env, jobject thiz)
{
    OsConstants_initConstants(env, thiz);
}
...
int register_info_guardianproject_libcore_io_OsConstants(JNIEnv* env) {
    jclass cls = (*env)->FindClass(env, "info/guardianproject/libcore/io/OsConstants");
    if (cls == NULL) {
