...
int create_test_file(sqlfs_t* sqlfs, char* filename, int size)
{
    ...
    randomdata[size-1] = '\0';
    return sqlfs_proc_write(sqlfs, filename, randomdata, size, 0, fi.flags);
}
...
void test_create_file_with_small_string(sqlfs_t *sqlfs)
{
    ...
    sqlfs_proc_write(sqlfs, "/bufdir/file", data, strlen(data), 0, fi.flags);
...
void test_write_seek_write(sqlfs_t *sqlfs)
{
    ...
    {
        assert(sqlfs_proc_write(sqlfs, testfilename, skip1, sz_skip1, 0, fi.flags));
        assert(sqlfs_proc_write(sqlfs, testfilename, skip2, sz_skip2, skip_offset, fi.flags));
        sqlfs_proc_getattr(sqlfs, testfilename, &sb);
...
static void wbb_helper(sqlfs_t *sqlfs, int testsize)
{
    ...
    assert(sqlfs_proc_write(sqlfs, testfilename, data, testsize, 0, fi.flags));
    sqlfs_proc_getattr(sqlfs, testfilename, &sb);
    assert(sb.st_size == testsize);
...
void test_o_append_existing_file(sqlfs_t *sqlfs)
{
    ...
    assert(rc == 0);
    sqlfs_proc_write(sqlfs, testfilename, buf, testsize, 0, ffi.flags);
    sqlfs_proc_getattr(sqlfs, testfilename, &sb);
